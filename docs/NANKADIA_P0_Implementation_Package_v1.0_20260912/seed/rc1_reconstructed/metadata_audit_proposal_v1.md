...
This packet records the batch metadata decision approved by the user on 2026-09-14. The canonical review CSV now contains the approved metadata and uses `HUMAN_GO`; this is not yet an immutable released Seed payload.
...
No field is inferred from a title alone. The values below were approved as a batch recommendation; release still requires the separate immutable Seed, hash, and coverage gates.
...
| ID | trajectory / detour / conflicts | duration / effort | environment / mobility / place | required resources | DQ | status |
|---|---|---|---|---|---:|---|
| SC-013 | ANY / NONE / [] | 10 / LOW | FLEXIBLE / NONE / CURRENT_PLACE | [] | 4 | PROPOSED |
...
SC-013 content is one self-contained mission: 「気になったものを1つ選び、写真または一言で残す。」There is no six-choice menu or random selection. The approved recommendation retains a 10-minute duration from the original micro-mission contract.
...
- Shared taxonomy and defaults: approved.
- All 36 CORE metadata rows: approved and copied to `core_candidate_review_v241.csv`.
- SC-013 expected duration: approved at 10 minutes.

Remaining release work is the immutable Seed migration, content hash, and situation-sensitive synthetic coverage gate.
